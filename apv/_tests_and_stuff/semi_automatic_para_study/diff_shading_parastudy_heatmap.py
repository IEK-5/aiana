...
import parameters
import utils
parameters = parameters.Parameters('fine')
df = utils.create_parameters_permutated_df(parameters)
# #
...
for sim_ID in df.index:
    ...
    df.loc[sim_ID, 'rel_dif_ir'] = transposition_ratio * (
        1-shading_loss) * 100  # %
df
...
def plot_heatmap(df: pd.DataFrame, x: str, y: str, z: str):
    ...
    cm = 'inferno'
    ax = sns.heatmap(
        data, annot=False, linewidths=0, ax=ax, cmap=cm, cbar_kws={'label': z})
    ax.invert_yaxis()
